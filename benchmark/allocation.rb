...
measure("baseline:    getbyte/[lo,hi].pack(\"CC\")+bytesplice  x#{N_SPLICE}") {
  buf = splice_src.dup
  N_SPLICE.times do |i|
    byte_i = (i * 10) >> 3
    shift  = (i * 10) & 7
...
measure("baseline:    getbyte pair + setbyte pair (manual mask/shift)  x#{N_SPLICE}") {
  buf = splice_src.dup
  N_SPLICE.times do |i|
    byte_i = (i * 10) >> 3
    shift  = (i * 10) & 7
...
measure("string_bits: bit_splice(i*10, 10, val)  x#{N_SPLICE}") {
  buf = splice_src.dup
  N_SPLICE.times do |i|

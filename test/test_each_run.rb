...
class TestCountRun < Minitest::Test
  ...
  def test_all_ones
    assert_equal 8, "\xFF".count_run(0)
  end

  def test_all_zeros
    assert_equal 8, "\x00".count_run(0)
  end

  def test_low_nibble_zeros
    # 0xF0 = 11110000: bits 0-3 are 0, bits 4-7 are 1
    assert_equal 4, "\xF0".count_run(0)
    assert_equal 4, "\xF0".count_run(4)
  end

  def test_single_set_bit
    # 0x01 = 00000001: bit 0 is 1, bit 1 is 0
    assert_equal 1, "\x01".count_run(0)
  end

  def test_cross_byte_run
    # "\xFF\xFF\x00": 16 ones then 8 zeros
    assert_equal 16, "\xFF\xFF\x00".count_run(0)
    assert_equal 8,  "\xFF\xFF\x00".count_run(16)
  end
  ...
  def test_run_starting_mid_byte
    ...
    assert_equal 2, "\xF0".count_run(2)
  end

  def test_out_of_range_returns_zero
    assert_equal 0, "\xFF".count_run(8)
    assert_equal 0, "\xFF".count_run(100)
  end

  def test_negative_returns_zero
    assert_equal 0, "\xFF".count_run(-1)
  end

  def test_bignum_returns_zero
    assert_equal 0, "\xFF".count_run(2**62)
  end

  def test_type_error
    assert_raises(TypeError) { "\xFF".count_run("0") }
    assert_raises(TypeError) { "\xFF".count_run(nil) }
  end
  ...
  def test_long_run_ones
    ...
    assert_equal 800, data.count_run(0)
  end
  ...
  def test_long_run_starting_mid_byte
    ...
    data = "\xF0" + "\xFF" * 99 + "\x0F"
    assert_equal 4, data.count_run(0)       # initial zeros
    assert_equal 796 + 4, data.count_run(4) # 796 ones in middle bytes + 4 in last byte
  end

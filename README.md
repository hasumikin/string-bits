...
Methods that walk a bitmap use `reverse:`. Methods that accept or return logical bit numbers use `lsb_first:`. This keeps traversal direction separate from intra-byte numbering, so `data.each_set_bit_offset(lsb_first: false).all? { |n| data.bit_at(n, lsb_first: false) }` stays true.
...
puts "\xAA".each_bit(reverse: true).to_a.inspect

...
class TestArrayMask < Minitest::Test
  ...
  def test_mask_lsb_default
    assert_equal [1, nil, 3, 4], DATA.mask(BITMAP_LSB)
  end
  ...
  def test_mask_bang_lsb
    ary = [1, 2, 3, 4]
    ary.mask!(BITMAP_LSB)
    assert_equal [1, nil, 3, 4], ary
  end

  def test_mask_bitmap_too_short
    assert_raises(ArgumentError) { [1, 2, 3, 4, 5, 6, 7, 8, 9].mask("\xFF") }
  end

  def test_mask_all_valid
    assert_equal [1, 2, 3, 4], DATA.mask("\xFF".b)
  end

  def test_mask_all_nil
    assert_equal [nil, nil, nil, nil], DATA.mask("\x00".b)
  end

  def test_mask_multi_byte
    data = (1..16).to_a
    bitmap = "\xFF\x00".b  # first 8 valid, next 8 nil
    result = data.mask(bitmap)
  ...
  def test_mask_integer_basic
    assert_equal [1, nil, 3, 4], DATA.mask(INT_BITMAP)
  end

  def test_mask_integer_all_valid
    assert_equal [1, 2, 3, 4], DATA.mask(0b1111)
  end

  def test_mask_integer_all_nil
    assert_equal [nil, nil, nil, nil], DATA.mask(0)
  end

  def test_mask_integer_invert
    # bits 0=1,1=0,2=1,3=1 -> inverted: 0=0,1=1,2=0,3=0
    assert_equal [nil, 2, nil, nil], DATA.mask(INT_BITMAP, invert: true)
  end

  def test_mask_integer_beyond_width_is_zero
    # integer 0b11 has only 2 set bits; elements 2,3 should be nil
    assert_equal [1, 2, nil, nil], DATA.mask(0b11)
  end
